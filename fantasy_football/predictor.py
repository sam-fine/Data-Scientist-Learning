...
class FantasyFootballPredictor:
    ...
    def check_data_current(self, events: pd.DataFrame):
        ...
        for gw in ["current", "next"]:
            ...
            if gw == "current":
                self.current_gw = pd.to_datetime(row["deadline_time"].values[0]).tz_convert("UTC")
    ...
    def clear_old_cache(self):
        '''
        If cache file is older than current gw, then remove it. Current gw being previous gw if we are in between gws.
        '''
        for fname in os.listdir(self.CACHE_DIR):
            ...
            # If file is older than the current GW deadline → delete it
            if file_date < self.current_gw:
                print(f"Deleting outdated cache file: {fname}")
                os.remove(fpath)
    ...
    def prediction_score(self, df: pd.DataFrame):
        ...
        """
        form_metrics = ["form_total_points", "form_expected_goals", "form_expected_assists", "form_minutes",
                        "difficulty", "points_per_game", "form"]
        # Safeguard for missing columns
        for col in form_metrics:
        ...
        norm_total_points = safe_norm("form_total_points")
        norm_expected_goals = safe_norm("form_expected_goals")
        norm_expected_assists = safe_norm("form_expected_assists")
        norm_difficulty = safe_norm("difficulty")


        # Heuristic weights – tweak as you like
        df["score"] = (
            0.40 * norm_total_points +                                   # recent FPL output
            0.30 * (norm_expected_goals + norm_expected_assists) +       # recent xG+xA
            0.15 * norm_minutes +                                        # nailedness
            0.10 * norm_ppg +                                            # season-long consistency
            0.05 * norm_form -                                           # FPL's built-in form metric
            0.10 * norm_difficulty                                       # penalty for harder fixture
        )
